...
function App() {
  ...
  const handlePageLinkChange = (event) => {
  ...
  const handleCustomize = () => {
    if (pageLink) {
      invoke("getCustomizationOptions", { pageLink })
        .then(({ length }) => {
          if (length < 250) {
  ...
  const handleStartQuiz = () => {
    ...
    const payload = { pageLink, numQuestions, questionType };
  ...
  const handleReview = () => {
    setReviewData(quizData); // Set the review data with the parsed quiz data
  };
    ...
    <div style={styles.container}>
      ...
      {!reviewData && !quizStarted && (
        <div style={styles.startContainer}>
          ...
          <p style={styles.subtitle}>
            Paste the link of the page you want to quiz on:
          </p>
          <input
            type="text"
            value={pageLink}
