...
async function extractContent(contentId) {
  if (!contentId) {
...
resolver.define("getCustomizationOptions", async (req) => {
  ...
  try {
    const { length } = await extractContent(contentId);
...
resolver.define("getText", async (req) => {
  const { contentId, numQuestions, questionType } = req.payload;
  ...
  try {
    ...
    const result = await extractContent(contentId);
  ...
  return axios
    ...
    .then((geminiResponse) => {
      ...
      try {
        ...
        return { response: JSON.stringify(parsedQuiz) , numQuestions: questionCount}; // Return the parsed quiz as JSON
      } catch (err) {
